    ...
    QLIST_FOREACH(active, &eclic->pending_list, next) {
        ...
        if (enable) {
             if (((trigger == 0) && new_intip) ||
                 ((trigger == 1) && (irq == active->irq) && !old_intip && new_intip) ||
                 ...
                 if (level >= eclic->mth) {
                    ...
                    riscv_cpu_eclic_interrupt(cpu, (active->irq & 0xFFF) | (shv << 12) | (level << 13));
                    free(active);
                    return;
                 }
             }
...
static void nuclei_eclic_realize(DeviceState *dev, Error **errp)
{
    ...
    cpu->env.exccode_pending = g_new0(uint32_t, 64);
    cpu->env.irq_start_idx = 0;
    cpu->env.irq_end_idx = 0;
